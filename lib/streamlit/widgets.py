...
class Widgets(object):
    def __init__(self):
        ...
        self._callbacks = {}  # type: Dict[str, WidgetCallback]

    def add_callback(self, widget_id: str, callback: WidgetCallback) -> None:
    ...
    def call_callbacks(self, new_widget_states: WidgetStates) -> None:
        ...
        for new_state in new_widget_states.widgets:
            callback = self._callbacks.get(new_state.id, None)
            if callback is None:
                # The widget does not have an onChanged callback.
                continue
